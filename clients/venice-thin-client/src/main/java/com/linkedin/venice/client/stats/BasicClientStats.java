...
public class BasicClientStats extends AbstractVeniceHttpStats {
  ...
  protected BasicClientStats(MetricsRepository metricsRepository, String storeName, RequestType requestType) {
    super(
        ...
        storeName,
        requestType);
    requestSensor = Lazy.of(() -> registerSensor("request", requestRate));
    Rate healthyRequestRate = new OccurrenceRate();
    healthySensor = Lazy.of(() -> registerSensor("healthy_request", healthyRequestRate));
    ...
    requestKeyCountSensor =
        Lazy.of(() -> registerSensor("request_key_count", requestKeyCountRate, new Avg(), new Max()));
    successRequestKeyCountSensor =
        Lazy.of(() -> registerSensor("success_request_key_count", successRequestKeyCountRate, new Avg(), new Max()));
